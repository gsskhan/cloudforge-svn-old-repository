...
import com.citi.retail.optima.etl.cache.resultset.setter.CollateralisedMappingResultSetExtractor;
 ...
 *
 */
public class CollateralisedMappingDaoImpl implements
		CacheLoaderDao<SegmentMappingDomain>, InitializingBean {

	private JdbcTemplate jdbcTemplate;
	private List<SegmentMappingDomain> collateralisedRefList;
	...
	public List<SegmentMappingDomain> getAgeDetails()
		...
		this.collateralisedRefList = new ArrayList<SegmentMappingDomain>();
		try {
			this.collateralisedRefList = this.jdbcTemplate.query(this.sql,
					new CollateralisedMappingResultSetExtractor());

		} catch (Exception e) {
			throw new OptimaRetailApplicationException(null, null, null,
					ServiceName.CACHE, ExceptionType.ERROR,
					"Exception occured while loading Collateralised Mapping",
