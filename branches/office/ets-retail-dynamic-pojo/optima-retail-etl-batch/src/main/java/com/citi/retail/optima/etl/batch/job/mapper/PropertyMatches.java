...
final class PropertyMatches {
	...
	private PropertyMatches(String propertyName, Class<?> beanClass,
		...
		possibleMatches = calculateMatches(
	...
	/**
	 * Build an error message for the given invalid property name, indicating
	 * the possible property matches.
	 */
	 ...
	 * Internally uses the <code>getStringDistance</code> method, which in turn
	 * uses the Levenshtein algorithm to determine the distance between two
	 * Strings.
	 * 
	...
	private String[] calculateMatches(PropertyDescriptor[] propertyDescriptors,
		...
		for (int i = 0; i < propertyDescriptors.length; i++) {
			if (propertyDescriptors[i].getWriteMethod() != null) {
				...
				int distance = calculateStringDistance(propertyName,
						possibleAlternative);
