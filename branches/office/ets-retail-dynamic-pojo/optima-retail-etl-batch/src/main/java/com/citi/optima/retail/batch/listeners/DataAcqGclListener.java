...
public class DataAcqGclListener<T> implements ItemProcessListener<T, T> , InitializingBean  {
	...
	public void afterProcess(T item, T result) {
		...
		if(dataMap != null){
			if(item.getClass().getSimpleName().contains("InMtg") && dataMap.containsKey(key)){
				dataMap.remove(key);
			}
