...
public class SoapCall {
	...
	private SOAPMessage soapCallservice(String endpointUrl, String requestNamespace, String soapAction,
		...
		try {
			...
			soapEnvelope.addNamespaceDeclaration("x20", requestNamespace);
            ...
            SOAPElement soapElementConsumerUserID = soapHeader.addChildElement("ConsumerUserID", "x20");
            soapElementConsumerUserID.addTextNode("ICAWP");
            SOAPElement soapElementConsumerName = soapHeader.addChildElement("ConsumerName", "x20");
            soapElementConsumerName.addTextNode("ICAWP");
            
            SOAPElement bodyElement = soapBody.addChildElement("getProfile","x20");            
            if (soeidList == null || soeidList.size() <=0 ) {
            	SOAPElement childElementWorkForceRequest = bodyElement.addChildElement("WorkForceRequest","x20");
            	SOAPElement childElementWorkforceName = childElementWorkForceRequest.addChildElement("WorkForceName","x20");
	        	SOAPElement childElementWorkforceFirstName = childElementWorkforceName.addChildElement("WorkForceFirstName","x20");
	        	childElementWorkforceFirstName.addTextNode(firstName);
	        	SOAPElement childElementWorkforceLastName = childElementWorkforceName.addChildElement("WorkForceLastName","x20");
	        	childElementWorkforceLastName.addTextNode(lastName);            	  			
    		} else {
    			SOAPElement childElementWorkForceRequest = bodyElement.addChildElement("WorkForceRequest","x20");
    			for (String soeid : soeidList) {    				
            		SOAPElement childElementSoeIdList = childElementWorkForceRequest.addChildElement("SOEIDList","x20");
	...
	private HashMap<String, String>[] parseGDWserviceReply(SOAPMessage replyMessage){		
		try {
			...
			if (body.hasFault()) {
				...
				String errorString = fault.getTextContent();
