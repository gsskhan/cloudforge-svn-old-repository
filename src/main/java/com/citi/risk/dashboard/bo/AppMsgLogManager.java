package com.citi.risk.dashboard.bo;

import java.util.Date;

import com.citi.risk.dashboard.exception.DashboardException;

/**
 * @author gk85985 This interface contain method to implement business logic,
 *         for maintaining data in table - "CLOUDAPP"."ETS_DSHBD_APP_MSG_LOG" .
 */

public interface AppMsgLogManager {

    void addRecordToAppMsgLog(String correlationID, long timestamp,
            String xmlString, Date msgDateTime) throws DashboardException;

}
