...
public class DataAcqHkMtgListener<T> implements ItemProcessListener<T, T> , InitializingBean{
	...
	public void afterProcess(T item, T result) {
		String key = null;
		try {
			key = (String) item.getClass().getMethod("getLnNo").invoke(item);
