...
public class CbFinanceSupportingDataProcessor <T> implements ItemProcessor<T, T>,InitializingBean, ItemReadListener<T> {
	...
	public T process(T currentEvent) throws Exception {
		preProcessdMap.put(((CbFinanceRecord)currentEvent).getPortfolioId(), currentEvent);
		return currentEvent;
	}
