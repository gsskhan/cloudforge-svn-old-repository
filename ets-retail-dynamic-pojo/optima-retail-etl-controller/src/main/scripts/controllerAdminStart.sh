...
then
	...
	then
		...
		sh ant -Dapp.dir="$APP_DIR" -DmaxMemory="$MAX_HEAP_SIZE" -Djmx.port="$JMX_PORT" -Dhttp.port="$HTTP_PORT" stop -f $ANT_BUILD_FILE 1> $ANT_CONFIG_DIR/controller-stop.log 2>&1 &
	fi	
	...
	sh ant -Dapp.dir="$APP_DIR" -DmaxMemory="$MAX_HEAP_SIZE" -Djmx.port="$JMX_PORT" -Dhttp.port="$HTTP_PORT" start -f $ANT_BUILD_FILE 1> $ANT_CONFIG_DIR/controller-start.log  2>&1 &
else
