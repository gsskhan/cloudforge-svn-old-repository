...
public class FICOSplScoreCacheLoaderImpl extends OptimaCacheDataLoader {

	private SingleValueCacheEntryHelper<String, FICOScoreVO> cacheEntryHelper;
	private FICOSplScoreDaoImpl dao;

	public void setDao(FICOSplScoreDaoImpl dao) {
		this.dao = dao;
	}
	...
	public Set<Entry<String, FICOScoreVO>> convertDataToMap(
		...
		for (FICOScoreVO refBand : refList) {
			baseRefMap.put(OptimaCacheUtil.keyCreator(
					refBand.getReportingPeriod(), refBand.getFicoScore()),
					refBand);
		}
